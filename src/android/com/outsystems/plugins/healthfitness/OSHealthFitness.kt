...
class OSHealthFitness : CordovaPlugin() {
        ...
        try {
            healthConnectViewModel.initAndRequestPermissions(
                ...
                privacyPolicyUrl = cordova.activity.resources.getString(
                    cordova.activity.resources.getIdentifier("privacy_policy_url", "string", cordova.activity.packageName)),
    ...
    private fun advancedQuery(args: JSONArray, callbackContext: CallbackContext) {
        var deprecationWarning: Pair<String, String>? = null
        val onDeprecatedUsage: () -> Unit = {
             deprecationWarning = Pair(OSHealthFitnessWarning.PERMISSIONS_DENIED_ERROR.code.toString()
                 , OSHealthFitnessWarning.PERMISSIONS_DENIED_ERROR.message)
        }
        val customGson = GsonBuilder().registerTypeAdapter(
            HealthEnumTimeUnit::class.java,
            TimeUnitSerializer(onDeprecatedUsage))
            .create()
        val parameters = customGson.fromJson(args.getString(0), HealthAdvancedQueryParameters::class.java)
    ...
    private fun onScheduleExactAlarmPermissionResult() {
        ...
        if (permissionDenied) {
            ...
            callbackContext?.let { sendError(it, HealthFitnessError.BACKGROUND_JOB_EXACT_ALARM_PERMISSION_DENIED_ERROR) }
            return
        }
        requestBackgroundJobPermissions()
    }
     ...
     */
    private fun setBackgroundJobWithParameters(parameters: BackgroundJobParameters, callbackContext: CallbackContext) {
        ...
        intent?.let {
            if (intent.getBooleanExtra(Constants.EXTRA_CONTAINS_READ_DATA_BACKGROUND, false)) {
                if (intent.getIntExtra(
                    ...
                    callbackContext?.let { setBackgroundJobWithParameters(backgroundParameters, it) }
                    return
                }
                callbackContext?.let { sendError(it, HealthFitnessError.BACKGROUND_JOB_READ_DATA_PERMISSION_DENIED) }
    ...
    fun areGooglePlayServicesAvailable(): Boolean {
        ...
        if (status != ConnectionResult.SUCCESS) {
            ...
            //callbackContext.sendPluginResult(null, result)
            return false
        }
        return true
    }
            ...
            BACKGROUND_JOB_PERMISSIONS_REQUEST_CODE -> {
                for (result in grantResults) {
                    if (result == PackageManager.PERMISSION_DENIED) {
                        callbackContext?.let { sendError(it,HealthFitnessError.BACKGROUND_JOB_PERMISSIONS_DENIED_ERROR ) }
                        return
                    }
                }
